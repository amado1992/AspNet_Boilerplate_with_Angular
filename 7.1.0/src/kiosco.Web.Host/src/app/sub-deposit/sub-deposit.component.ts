...
@Component({
    selector: 'app-sub-deposit',
    templateUrl: './sub-deposit.component.html',
    styleUrls: ['./sub-deposit.component.css'],
    animations: [appModuleAnimation()]
})

export class SubDepositComponent extends AppComponentBase implements OnChanges, OnInit, AfterViewInit, DoCheck {
    totalRows = 0;
    pageSize = 10;
    currentPage = 0;
    ...
    @Output() changeInfoSub: EventEmitter<any> = new EventEmitter();
    ...
    selectBaseFee(event: Event) {
        ...
        this.appBaseService.postBaseFee('FloorDanceType', this.baseFee)
            .subscribe(next => {
                this.changeInfoSub.emit(next.result)
            }, (error: HttpErrorResponse) => {
                let errorMessage = '';
    ...
    getTypeBusinessElements() {
        this.appBaseService.getSubTypeBusinessElements('TypeBusinessElement', 0, 200000, this.filter, this.order, this.group.id)
            .subscribe(response => {
                if (response.result) {
