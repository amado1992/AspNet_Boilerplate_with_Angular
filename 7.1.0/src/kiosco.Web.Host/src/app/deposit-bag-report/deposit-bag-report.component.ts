import { AfterViewInit, ChangeDetectorRef, Component, DoCheck, EventEmitter, Injector, Input, OnChanges, OnInit, Output, SimpleChanges, ViewChild } from '@angular/core';
...
import { AccountingDto } from '@shared/entities/accounting';
...
@Component({
    selector: 'app-deposit-bag-report',
    templateUrl: './deposit-bag-report.component.html',
    styleUrls: ['./deposit-bag-report.component.css'],
    animations: [appModuleAnimation()]
})

export class DepositBagReportComponent extends AppComponentBase implements OnChanges, OnInit, AfterViewInit, DoCheck {
    totalRows = 0;
    pageSize = 10;
    currentPage = 0;
    ...
    showingDueId = new FormControl(0)
    ...
    txtQuery: any; // bind this to input with ngModel
    txtQueryChanged: Subject<any> = new Subject<any>();
    counts: AccountingDto[] = []
    accountings: any[] = []
    addCounts: any[] = []
    addCredit: any[] = []
    addCash: any[] = []
    isOperation = true
    isOperationRevenues = true
    revenuesResult: AccountingDto[] = []
    ...
    constructor(private appBaseService: AppCrudBaseService, public dialog: MatDialog, injector: Injector, private cd: ChangeDetectorRef, private fb: FormBuilder, public datepipe: DatePipe) {
        ...
        this.accountingDto = new AccountingDto()
    ...
    ngOnChanges(changes: SimpleChanges) {
        ...
        /*this.shiftObj = this.shift
        if (this.shift != undefined) {
            this.shiftId.setValue(JSON.parse(this.shift.id))
        }
        this.ngOnInit()*/
    }
    ngOnInit() {
        ...
        this.getOfMain()
        //this.getOfMainWaitressRevenues()
        this.getTypeBusinessElements()
        this.pricesShifts()
        this.getGroupTotal()
        //this.getSectionTotal()
    }
    ngDoCheck() {
                    ...
                    if (shifId) {
                        ...
                        for (let entry of this.shiftsResults) {
                            ...
                            if ((date.getHours() >= 12 && date.getHours() <= 19)
                                ...
                                this.schedule = this.scheduleDay
                                //localStorage.setItem("shiftId", entry.id)
                            }
    ...
    quotaV2() {
        ...
        if (this.shiftId.value != 0 && this.element != undefined) {
            this.appBaseService.getQuotaId('FloorDanceType', this.currentPage * this.pageSize, this.pageSize, this.filter, this.order, this.element.id, this.shiftId.value)
                .subscribe(response => {
                    if (response.result) {
                        this.quotaResut = response.result
    ...
    scheduleDance(/*danceId?, start?, end?, scheduleId?*/) {
        ...
        if (this.element != undefined) {
            ...
            this.appBaseService.getSheduleDanceId('FloorDanceType', this.currentPage * this.pageSize, this.pageSize, this.filter, this.order, this.element.id, this.shiftId.value)
                .subscribe(response => {
                    if (response.result) {
    ...
    countDanceObj(posSchedule, dance) {
        ...
        var id
        if (this.schedule != undefined) {
            var id = this.schedule[posSchedule].id
        }
    ...
    getTypeBusinessElements() {
        this.appBaseService.getTypeBusinessElements('TypeBusinessElement', 0, 200000, this.filter, this.order, this.group.id)
            .subscribe(response => {
                if (response.result) {
    ...
    getOfMain() {
        this.appBaseService.getOfMain('SubBusinessElement', 0, 200000, this.filter, this.order, this.group.id)
            .subscribe(response => {
                if (response.result) {
                    this.subs = response.result.items;
    ...
    getOfMainWaitressRevenues() {
        this.appBaseService.getOfMain('WaitressRevenues', 0, 200000, this.filter, this.order, this.group.id)
            .subscribe(response => {
                if (response.result) {
                    this.waitressRevenues = response.result.items;
    ...
    getGroupTotal() {
        this.appBaseService.getGroupTotal('TypeBusinessElement', 0, 200000, this.filter, this.order, this.group.id, this.shiftId.value)
            .subscribe(response => {
                if (response.result) {

                    this.groupTotal = response.result;
                }
            }, (error: HttpErrorResponse) => {
                let errorMessage = '';
    ...
    getSectionTotal() {
        this.appBaseService.getSectionTotal('TypeBusinessElement', 0, 200000, this.filter, this.order, this.section.id, this.shiftId.value)
            .subscribe(response => {
                if (response.result) {

                    this.sectionTotal = response.result;
    ...
    pricesShifts() {
        this.appBaseService.list('PriceShift', 0, 200000, this.filter, this.order)
            .subscribe(response => {
                if (response.result) {
                    this.prices = response.result.items;
                }
            }, (error: HttpErrorResponse) => {
                let errorMessage = '';
    ...
    price(value) {
        var price = 0
        var obj = this.prices.find(val => val.typeBusinessElementId == value)
    ...
    onFieldChange(e, type, i) {
        var start = Date.now()
        var today = new Date(start)
        var latest_date = this.datepipe.transform(today, 'yyyy-MM-dd HH:mm:ss');
        var price = 0
        var obj = this.prices.find(val => val.typeBusinessElementId == type)

        if (obj != undefined) {
            price = obj.price
        }

        this.accountingDto.typeBusinessElementId = type
        if (this.price(type) > 0) {
            this.accountingDto.total = e * price
        } else {
            this.accountingDto.total = e
        }

        this.accountingDto.date = latest_date
        this.accountingDto.shiftId = this.shiftId.value
        this.accountingDto.count = e
        var entity = this.accountings.find(val => val.typeBusinessElementId == type)
        if (entity != undefined) {
            this.isOperation = false
            this.accountingDto.id = entity.id
        } else {
    ...
    onFieldChangeCash(money, id) {
        var start = Date.now()
        var today = new Date(start)
        var latest_date = this.datepipe.transform(today, 'yyyy-MM-dd HH:mm:ss');

        var entity = this.accountings.find(val => val.waitressRevenuesId == id)

        this.accountingDto.cash = money
        this.accountingDto.date = latest_date
        this.accountingDto.shiftId = this.shiftId.value
        this.accountingDto.waitressRevenuesId = id

        if (entity != undefined) {
            this.accountingDto.credit = entity.credit
            this.isOperation = false
            this.accountingDto.id = entity.id
        } else {
            this.isOperation = true
    ...
    onFieldChangeCredit(money, id) {
        var start = Date.now()
        var today = new Date(start)
        var latest_date = this.datepipe.transform(today, 'yyyy-MM-dd HH:mm:ss');
        var entity = this.accountings.find(val => val.waitressRevenuesId == id)

        this.accountingDto.credit = money
        this.accountingDto.date = latest_date
        this.accountingDto.shiftId = this.shiftId.value
        this.accountingDto.waitressRevenuesId = id

        if (entity != undefined) {
            this.accountingDto.cash = entity.cash
            this.isOperation = false
            this.accountingDto.id = entity.id
        } else {
            this.isOperation = true
    ...
    accountingSave(): void {
        ...
        if (this.isOperation) {
            ...
            this.appBaseService.create('AccountingPerShift', this.accountingDto)
                .subscribe(response => {
                    this.isLoadingResults = false
                    abp.notify.info("New element successfully created.")
                    this.changeInfo.emit(true)
            ...
            this.appBaseService.update('AccountingPerShift', this.accountingDto)
                .subscribe(response => {
                    this.isLoadingResults = false
    ...
    getAccountings() {
        this.appBaseService.getAccountings('AccountingPerShift', 0, 200000, this.filter, this.order, this.shiftId.value)
            .subscribe(response => {
                if (response.result) {
                    this.accountings = response.result.items;
                    for (let entry of this.types) {
                        var entity = this.accountings.find(val => val.typeBusinessElementId == entry.id)
                        if (entity != null) {
                            this.counts.push(entity)
                        } else {
                            //this.counts.push(null)
                            var obj = new AccountingDto()
                            this.counts.push(obj)
                        }
                    }

                    this.loadWaitressRevenues()
    ...
    loadWaitressRevenues() {
        for (let entry of this.waitressRevenues) {
            var entity = this.accountings.find(val => val.waitressRevenuesId == entry.id)
            if (entity != null) {
                this.revenuesResult.push(entity)
            } else {
                //this.counts.push(null)
                var obj = new AccountingDto()
                this.revenuesResult.push(obj)
            }
        }
    }
