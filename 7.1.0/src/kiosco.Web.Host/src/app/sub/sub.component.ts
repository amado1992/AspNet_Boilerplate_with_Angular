...
export class SubComponent extends AppComponentBase implements OnInit, AfterViewInit {
    totalRows = 0;
    pageSize = 10;
    currentPage = 0;
    ...
    displayedColumns = ['name', 'mainBusinessElementName', 'actions'];
    ...
    openCreateDialog(): void {
        ...
        let dialogRef = this.dialog.open(SubCEComponent, config);
    ...
    onDeleteClick(entry): void {
        abp.message.confirm("Are you sure you want to delete this information?.",
            ...
            (result: boolean) => {
                if (result) {
                    this.appBaseService.delete("SubBusinessElement", entry)
                        .subscribe(response => {
                            if (response.success) {
    ...
    loadData() {
        ...
        this.appBaseService.list('SubBusinessElement', this.currentPage * this.pageSize, this.pageSize, this.filter, this.order)
            .subscribe(response => {
                if (response.result) {
