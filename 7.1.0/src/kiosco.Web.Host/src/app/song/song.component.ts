...
import { SongCEComponent } from "../song/songCE.component";
...
export class SongComponent extends AppComponentBase implements OnInit, AfterViewInit {
    totalRows = 0;
    pageSize = 10;
    currentPage = 0;
    ...
    displayedColumns = ['title', 'time', 'actions'];
    ...
    openCreateDialog(): void {
        ...
        let dialogRef = this.dialog.open(SongCEComponent, config);
    ...
    onDeleteClick(entry): void {
        abp.message.confirm("Are you sure you want to delete this information?.",
            ...
            (result: boolean) => {
                if (result) {
                    this.appBaseService.delete("Song", entry)
                        .subscribe(response => {
                            if (response.success) {
    ...
    loadData() {
        ...
        this.appBaseService.list('Song', this.currentPage * this.pageSize, this.pageSize, this.filter, this.order)
            .subscribe(response => {
                if (response.result) {
