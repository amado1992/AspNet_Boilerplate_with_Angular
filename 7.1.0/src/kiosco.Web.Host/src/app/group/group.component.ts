...
import { GroupCEComponent } from "../group/groupCE.component";
...
export class GroupComponent extends AppComponentBase implements OnInit, AfterViewInit {
    ...
    displayedColumns = ['name', 'sectionName', 'actions'];
    ...
    openCreateDialog(): void {
        ...
        let dialogRef = this.dialog.open(GroupCEComponent, config);
    ...
    onDeleteClick(entry): void {
        abp.message.confirm("Are you sure you want to delete this information?.",
            ...
            (result: boolean) => {
                if (result) {
                    this.appBaseService.delete("MainBusinessElement", entry)
                        .subscribe(response => {
                            if (response.success) {
    ...
    loadData() {
        ...
        this.appBaseService.list('MainBusinessElement', this.currentPage * this.pageSize, this.pageSize, this.filter, this.order)
            .subscribe(response => {
                if (response.result) {
