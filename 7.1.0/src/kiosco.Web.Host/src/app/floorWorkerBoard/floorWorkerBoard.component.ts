...
import { BoardAccountingComponent } from '@app/board-accounting/board-accounting.component';

@Component({
    selector: 'app-floorWorkerBoard',
    templateUrl: './floorWorkerBoard.component.html',
    styleUrls: ['./floorWorkerBoard.component.css'],
    animations: [appModuleAnimation()]
})

export class FloorWorkerBoardComponent extends AppComponentBase implements OnInit, AfterViewInit {
    totalRows = 0;
    pageSize = 10;
    currentPage = 0;
    ...
    print() {
        console.log("PRINTER IMG 2");
        this.captureService.getImage(this.screen.nativeElement, true)
            .pipe(
                tap(img => {
                    console.log("PRINTER IMG ",img);
                })
            ).subscribe();
    }

    openDepositBagDialog(): void {
        let config: MatDialogConfig = {
            disableClose: false,
            width: '1200px',
            height: '',
        ...
        let dialogRef = this.dialog.open(BoardAccountingComponent, config);
        dialogRef.afterClosed().subscribe(result => {
            if (result) {
                this.loadData();
            }
