...
export class BlackFormComponent extends AppComponentBase implements OnInit, AfterViewInit {
    totalRows = 0;
    pageSize = 10;
    currentPage = 0;
    ...
    displayedColumns = ['name', 'actions', 'download'];
    ...
    openCreateDialog(): void {
        ...
        let dialogRef = this.dialog.open(BlackFormCEComponent, config);
    ...
    onDeleteClick(entry): void {
        abp.message.confirm("Are you sure you want to delete this information?.",
            ...
            (result: boolean) => {
                if (result) {
                    this.appBaseService.delete("DocumentWhite", entry)
                        .subscribe(response => {
                            if (response.success) {
    ...
    loadData() {
        ...
        this.appBaseService.list('DocumentWhite', this.currentPage * this.pageSize, this.pageSize, this.filter, this.order)
            .subscribe(response => {
                if (response.result) {
    ...
    downloadFile(id, fileName){
        this.appBaseService.getFile('DocumentWhite', id)
        .subscribe(response => {
