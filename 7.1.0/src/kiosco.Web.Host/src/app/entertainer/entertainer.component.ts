...
import { EntertainerCEComponent } from "../entertainer/entertainerCE.component";
...
export class EntertainerComponent extends AppComponentBase implements OnInit, AfterViewInit {
    totalRows = 0;
    pageSize = 10;
    currentPage = 0;
    ...
    displayedColumns = ['firstName', 'lastName', 'address', 'actions', 'upload', 'download'];
    ...
    openCreateDialog(): void {
        ...
        let dialogRef = this.dialog.open(EntertainerCEComponent, config);
    ...
    openUploadDialog(entry): void {
        ...
        let dialogRef = this.dialog.open(UploadFileCEComponent, config);
        dialogRef.afterClosed().subscribe(result => {
            if (result) {
                this.loadData();
            }
    ...
    openDownloadDialog(entry): void {
        ...
        let dialogRef = this.dialog.open(DownloadFileCEComponent, config);
        dialogRef.afterClosed().subscribe(result => {
            if (result) {
                this.loadData();
            }
    ...
    onDeleteClick(entry): void {
        abp.message.confirm("Are you sure you want to delete this information?.",
            ...
            (result: boolean) => {
                if (result) {
                    this.appBaseService.delete("Entertainer", entry)
                        .subscribe(response => {
                            if (response.success) {
    ...
    loadData() {
        ...
        this.appBaseService.list('Entertainer', this.currentPage * this.pageSize, this.pageSize, this.filter, this.order)
            .subscribe(response => {
                if (response.result) {
