...
import { KeyCEComponent } from "../key/keyCE.component";
...
export class KeyComponent extends AppComponentBase implements OnInit, AfterViewInit {
    totalRows = 0;
    pageSize = 10;
    currentPage = 0;
    ...
    displayedColumns = ['name', 'mainBusinessElementName', 'subBusinessElementName', 'actions'];
    ...
    openCreateDialog(): void {
        ...
        let dialogRef = this.dialog.open(KeyCEComponent, config);
    ...
    onDeleteClick(entry): void {
        abp.message.confirm("Are you sure you want to delete this information?.",
            ...
            (result: boolean) => {
                if (result) {
                    this.appBaseService.delete("TypeBusinessElement", entry)
                        .subscribe(response => {
                            if (response.success) {
    ...
    loadData() {
        ...
        this.appBaseService.list('TypeBusinessElement', this.currentPage * this.pageSize, this.pageSize, this.filter, this.order)
            .subscribe(response => {
                if (response.result) {
