...
@Component({
    selector: 'app-waitress-revenues',
    templateUrl: './waitress-revenues.component.html',
    styleUrls: ['./waitress-revenues.component.css'],
    animations: [appModuleAnimation()]
})

export class WaitressRevenuesComponent extends AppComponentBase implements OnInit, AfterViewInit {
    totalRows = 0;
    pageSize = 10;
    currentPage = 0;
    ...
    openCreateDialog(): void {
        ...
        let dialogRef = this.dialog.open(WaitressRevenuesCEComponent, config);
    ...
    onDeleteClick(entry): void {
        abp.message.confirm("Are you sure you want to delete this information?.",
            ...
            (result: boolean) => {
                if (result) {
                    this.appBaseService.delete("WaitressRevenues", entry)
                        .subscribe(response => {
                            if (response.success) {
    ...
    loadData() {
        ...
        this.appBaseService.list('WaitressRevenues', this.currentPage * this.pageSize, this.pageSize, this.filter, this.order)
            .subscribe(response => {
                if (response.result) {
