...
import { ShiftCEComponent } from "../shift/shiftCE.component";
...
export class ShiftComponent extends AppComponentBase implements OnInit, AfterViewInit {
    totalRows = 0;
    pageSize = 10;
    currentPage = 0;
    ...
    openCreateDialog(): void {
        ...
        let dialogRef = this.dialog.open(ShiftCEComponent, config);
    ...
    onDeleteClick(entry): void {
        var shiftId = JSON.parse(localStorage.getItem("shiftId"))

        abp.message.confirm("Are you sure you want to delete this information?.",
            "Information",
            (result: boolean) => {
                if (result) {
                    this.appBaseService.delete("Shift", entry)
                        .subscribe(response => {
                            if (response.success) {
                                 
                                if (shiftId) {
                                    var result = this.shiftsResults.some(val => val.id == shiftId);
                                    if(result == false){
