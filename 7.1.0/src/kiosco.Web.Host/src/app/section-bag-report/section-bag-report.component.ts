...
@Component({
    selector: 'app-section-bag-report',
    templateUrl: './section-bag-report.component.html',
    styleUrls: ['./section-bag-report.component.css'],
    animations: [appModuleAnimation()]
})

export class SectionBagReportComponent extends AppComponentBase implements OnChanges, OnInit, AfterViewInit, DoCheck {
    totalRows = 0;
    pageSize = 10;
    currentPage = 0;
    ...
    //fin temporizador
    sectionTotal: any
    @Output() changeInfoGroup: EventEmitter<any> = new EventEmitter();
    @Output() changeInfoSection: EventEmitter<any> = new EventEmitter();
    ...
    ngOnInit() {
        console.log("LOG SECTION... ", localStorage.getItem("shiftIdReport"))
    ...
    getMainBusinessElements() {
        this.appBaseService.GetMainBusinessElements('MainBusinessElement', 0, 200000, this.filter, this.order, this.section.id)
            .subscribe(response => {
                if (response.result) {
    ...
    changeInfo(value) {
        if (value) {
            this.changeInfoGroup.emit(true)
        }
    }

    getSectionTotal() {
        //console.log("LOG EER ", localStorage.getItem("shiftIdReport"))
        var _shiftId
        if (this.shift == undefined) {
            ...
            if (shiftId) {
                ...
                _shiftId = JSON.parse(_shiftId)
            }
        } else {
            _shiftId = this.shift.id

            _shiftId = JSON.parse(_shiftId)
        }
        //if (this.section != undefined && this.shift != undefined) {
            this.appBaseService.getSectionTotal('TypeBusinessElement', 0, 200000, this.filter, this.order, this.section.id, _shiftId)
                .subscribe(response => {
                    if (response.result) {

                        this.sectionTotal = response.result;
                        this.changeInfoSection.emit(this.sectionTotal)
