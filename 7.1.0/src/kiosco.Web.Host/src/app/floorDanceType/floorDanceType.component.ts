...
@Component({
    selector: 'app-floorDanceType',
    templateUrl: './floorDanceType.component.html',
    styleUrls: ['./floorDanceType.component.css'],
    animations: [appModuleAnimation()]
})

export class FloorDanceTypeComponent extends AppComponentBase implements OnInit, AfterViewInit {
    totalRows = 0;
    pageSize = 10;
    currentPage = 0;
    ...
    displayedColumns = ['activityTodayEntertainerFirstName', 'activityTodayEntertainerLastName', 'activityTodayEntertainerStageName', 'activityTodayEntertainerMobilePhone', 'danceTypeTitle', 'shiftTitle', 'timeWorked', /*'keyGroupName', 'keyName',*/ 'start', /*'end',*/ 'actions'];
    ...
    loadData() {
        ...
        this.appBaseService.list('FloorDanceType', this.currentPage * this.pageSize, this.pageSize, this.filter, this.order)
            .subscribe(response => {
                if (response.result) {
    ...
    loadFilter() {
        ...
        this.appBaseService.listFilter('FloorDanceType', this.currentPage * this.pageSize, this.pageSize, this.filter, this.order, this.filterDate)
            .subscribe(response => {
                if (response.result) {
                    this.dataSource = new MatTableDataSource<any>(response.result.items);
                    this.paginator.pageIndex = this.currentPage;
