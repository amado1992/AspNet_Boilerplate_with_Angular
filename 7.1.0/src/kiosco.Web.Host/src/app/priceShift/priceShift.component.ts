...
import { PriceShiftCEComponent } from "../priceShift/priceShiftCE.component";
import { AppComponentBase } from '../../shared/app-component-base';
import { HttpErrorResponse } from '@angular/common/http';

@Component({
    selector: 'app-priceShift',
    templateUrl: './priceShift.component.html',
    styleUrls: ['./priceShift.component.css'],
    animations: [appModuleAnimation()]
})

export class PriceShiftComponent extends AppComponentBase implements OnInit, AfterViewInit {
    totalRows = 0;
    pageSize = 10;
    currentPage = 0;
    ...
    displayedColumns = ['keyGroupName', 'keyName', 'shiftTitle', 'price', 'percent', 'actions'];
    ...
    openCreateDialog(): void {
        ...
        let dialogRef = this.dialog.open(PriceShiftCEComponent, config);
    ...
    onDeleteClick(entry): void {
        abp.message.confirm("Are you sure you want to delete this information?.",
            ...
            (result: boolean) => {
                if (result) {
                    this.appBaseService.delete("PriceShift", entry)
                        .subscribe(response => {
                            if (response.success) {
    ...
    loadData() {
        ...
        this.appBaseService.list('PriceShift', this.currentPage * this.pageSize, this.pageSize, this.filter, this.order)
            .subscribe(response => {
                if (response.result) {
